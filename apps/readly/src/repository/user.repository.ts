...
export const getUserInfoData = async (id: string) => {
  const { data } = await readlyApiAxiosInstance().get<
    UserInfoDto & { is_my: boolean }
  >('/user/info', { params: { user_id: id } });
  return data;
};

export const getUserMyInfoData = async () => {
  const { data } = await readlyApiAxiosInstance().get<UserInfoDto | null>(
    '/user/my-info',
  );
  return data;
};
